  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.wrapper}>
        <h1 style={styles.heading}>Find Your Perfect Style</h1>
        <p style={styles.subheading}>
          Discover outfits tailored to your unique features and preferences.
        </p>

        <div style={styles.filterBox}>
          <h3 style={styles.filterTitle}>Personalize Your Search</h3>
          <div style={styles.filterGrid}>
            <select name="skin_tone" onChange={handleChange} style={styles.select}>
            ...
            <select name="gender" onChange={handleChange} style={styles.select}>
              <option value="">Gender</option>
              <option>Male</option>
              <option>Female</option>
            </select>

            <select name="body_type" onChange={handleChange} style={styles.select}>
            ...
            <select name="occasion" onChange={handleChange} style={styles.select}>
            ...
            <select name="theme" onChange={handleChange} style={styles.select}>
          ...
          <div style={styles.sortRow}>
            <select name="sort" style={styles.select}>
              <option>Most Popular</option>
              <option>Price: Low to High</option>
              <option>Price: High to Low</option>
            </select>
            <button onClick={fetchTopLikedOutfits} style={styles.button}>
              🔍 Find Outfits
            </button>
          </div>
        ...
        <div style={styles.outfitsContainer}>
          {topLikedOutfits.length > 0 ? (
            topLikedOutfits.map((outfit, index) => (
              <div key={index} style={styles.outfitCard}>
                <img
                  src={outfit.pic_url}
                  alt="Outfit"
                ...
                <div style={styles.outfitDetails}>
                  <h3 style={styles.outfitTitle}>
                    {outfit.occasion} - {outfit.theme}
                  </h3>
                  <p>{outfit.gender} • {outfit.body_type}</p>
                  <p>Skin Tone: {outfit.skin_tone}</p>
                  <p>Height: {outfit.height} cm</p>
                  <p style={styles.likes}>❤ {outfit.likes_count}</p>
                  <button style={styles.detailsButton} onClick={() => setSelectedOutfit(outfit)}>
                    View More Details
                  </button>
                </div>
              </div>
            ...
            <p style={styles.noResults}>No outfits found.</p>
          )}
        </div>
        {/* --- Static Cards Grid (3 rows, 3 per row) --- */}
<h2 style={{ textAlign: "left", margin: "40px 0 15px", fontSize: "1.5rem", fontWeight: "600" }}>9 Outfits Found</h2>
<div style={{
  display: "grid",
  ...
  {[
    ...
    <div key={`card-${index}`} style={{
      ...
      />
      <h3 style={{ fontSize: "1rem", fontWeight: "600", marginBottom: "5px" }}>{outfit.occasion} Style</h3>
      <p style={{ margin: "0", fontWeight: "500", color: "#555" }}>{outfit.name}</p>
      <p style={{ margin: "5px 0", fontSize: "0.9rem", color: "#666" }}>{outfit.gender} • {outfit.body_type}</p>
      ...
      >
        ...
        {selectedOutfit && (
          <div style={styles.modalOverlay} onClick={() => setSelectedOutfit(null)}>
            <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
              <h2>{selectedOutfit.occasion} {selectedOutfit.theme} Outfit</h2>
              <p style={styles.subheading}>Complete outfit details and specifications</p>
              <div style={styles.modalMain}>
                <img src={selectedOutfit.pic_url} alt="Outfit" style={styles.modalImage} />
                <div>
                  <h3 style={styles.modalTitle}>{selectedOutfit.name || "Style Info"}</h3>
              ...
              <button onClick={() => setSelectedOutfit(null)} style={styles.modalClose}>Close</button>
            </div>
          </div>
        )}
      </div>
